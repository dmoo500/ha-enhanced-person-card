...
class EnhancedPersonCard extends HTMLElement {
    constructor() {
        ...
        this._lastEntityAttributes = null;
        this._lastDevicesData = [];
        this._lastConfigHash = '';
        this._rendered = false;
        this._usedSensors = new Set(); // Track sensors that have already been assigned to prevent cross-device matches
    ...
    _getAttributeIcon(attributeName, value) {
        switch (attributeName) {
            case 'battery_level':
                ...
                if (batteryLevel > 80)
                    return { icon: 'mdi:battery', color: '#4caf50' };
                if (batteryLevel > 60)
            ...
            case 'speed':
                ...
                if (speed > 50)
                    return { icon: 'mdi:speedometer', color: '#f44336' };
                if (speed > 10)
    ...
    _getAttributeEntriesForDevice(deviceEntity, attrName) {
        const results = [];
        ...
        Object.values(this._hass.states).forEach((entity) => {
            ...
            if (seenEntities.has(entityDisplayName))
                return;
            ...
            const sensorId = entity.entity_id.toLowerCase();
            const sensorName = (entity.attributes.friendly_name || '').toLowerCase();
            const hasAttributeMatch = patterns.some(pattern => sensorId.includes(pattern) || sensorName.includes(pattern));
            ...
            const isExcluded = excludePatterns.some(exclude => sensorId.includes(exclude) || sensorName.includes(exclude));
            if (isExcluded) {
                return;
            }
                ...
                item.addEventListener('click', (e) => {
                    ...
                    if (entityId) {
                        ...
                        if (attributeName === 'battery_level') {
                            ...
                            const batterySensor = Object.keys(this._hass.states).find(id => id.includes('battery') && id.includes(deviceName));
                            if (batterySensor) {
